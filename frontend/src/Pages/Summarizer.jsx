...
const generateColors = (count) =>
  Array.from({ length: count }, (_, i) => colorPalette[i % colorPalette.length]);
...
export default function SummarizerLayout() {
  ...
  // ─── Fetch files (token auto-attached via api instance) ───
  ...
  useEffect(() => {
    ...
    tempDiv.childNodes.forEach((node) => {
      if (node.tagName === "TABLE") {
        ...
        const headers =
          rows[0] && Array.from(rows[0].querySelectorAll("th")).length > 0
  ...
  const downloadPDF = () => {
    ...
    link.href = url;
    link.download = selectedFile?.originalName || "summary.pdf";
    link.click();
  };
  ...
  const extractPieData = (tableData) => {
    ...
    for (let i = 1; i < tableData.headers.length; i++) {
      if (tableData.rows.some((r) => String(r[i] || "").includes("%"))) { percentCol = i; break; }
    }
    const rows = tableData.rows
      .map((r) => ({ label: r[0]?.trim() || "Unknown", value: parsePercentage(percentCol !== -1 ? r[percentCol] : r[1]) }))
      .filter((r) => r.value > 0);
          ...
          <thead>
            <tr className="bg-gradient-to-r from-blue-600 to-blue-700">
              {tableData.headers.map((h, i) => (
                <th key={i} className="px-4 py-2.5 text-xs font-semibold text-left text-white whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {tableData.rows.map((row, rIdx) => (
              <tr key={rIdx} className={`transition-colors ${rIdx % 2 === 0 ? "bg-white" : "bg-blue-50/40"} hover:bg-blue-50`}>
                {row.map((cell, cIdx) => (
                  <td key={cIdx} className={`px-4 py-2 text-xs border-t border-gray-100 ${cIdx === 0 ? "font-medium text-gray-800" : "text-gray-600"}`}>{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
  ...
  const renderChartFromTable = (tableData, index, chartTypes) => {
    ...
    if (type === "pie-shareholding") {
      ...
      return (
        <div key={`chart-${index}`} className="flex flex-col items-center my-6">
          <div className="w-full max-w-[600px] h-[420px] p-6 bg-white rounded-2xl shadow-md border border-gray-100 flex justify-center items-center">
            <Pie
              data={{
                labels: pieData.map((r) => r.label),
                datasets: [{ data: pieData.map((r) => parseFloat(r.value.toFixed(2))), backgroundColor: generateColors(pieData.length), borderColor: "#fff", borderWidth: 2, hoverOffset: 14 }],
              }}
              options={{ ...baseChartOptions, plugins: { ...baseChartOptions.plugins, legend: { position: "bottom", labels: { boxWidth: 14, font: { size: 13 }, padding: 12 } } } }}
            />
          </div>
        </div>
      );
    ...
    if (type === "bar-revenue") {
      ...
      const validRows = tableData.rows
        .map((r) => {
          const year = r[yearIndex]?.trim() || lastYear;
          if (r[yearIndex]?.trim()) lastYear = r[yearIndex].trim();
          return { year, segment: r[segmentIndex]?.trim(), revenue: cleanValue(r[revenueIndex]) };
        })
        .filter((r) => r.year && r.segment && !/%/i.test(r.segment) && r.revenue > 0);

      const segments = [...new Set(validRows.map((r) => r.segment))];
      const years = [...new Set(validRows.map((r) => r.year))];
      const datasets = years.map((year, i) => ({
        label: year,
        data: segments.map((segment) => { const m = validRows.find((r) => r.year === year && r.segment === segment); return m ? m.revenue : 0; }),
        backgroundColor: colorPalette[i % colorPalette.length],
        borderRadius: 6,
      }));

      return (
        <div key={`revenue-${index}`} className="p-5 mx-auto my-4 bg-white border border-gray-100 shadow-md rounded-2xl" style={{ maxWidth: 900 }}>
          <p className="mb-3 font-semibold text-center text-gray-700">Revenue Breakdown — Segment-wise Year Comparison (₹ Cr)</p>
          <div className="w-full h-[360px]">
            <Bar data={{ labels: segments, datasets }} options={{ ...baseChartOptions, plugins: { ...baseChartOptions.plugins, legend: { position: "bottom", labels: { font: { size: 12 }, boxWidth: 14 } } }, scales: { x: { ticks: { font: { size: 11 }, maxRotation: 45, minRotation: 45 }, grid: { display: false } }, y: { beginAtZero: true, ticks: { font: { size: 11 } } } } }} />
          </div>
        </div>
      );
    }
    ...
    if (type === "bar-financial") {
      ...
      const charts = rowsToChart
        .map((row, rIdx) => {
          const values = years.map((h) => cleanValue(row[tableData.headers.indexOf(h)]));
          if (!values.some((v) => v !== 0)) return null;
          return (
            <div key={rIdx} className="flex flex-col items-center p-4 bg-white border border-gray-100 shadow rounded-xl" style={{ maxWidth: 420, margin: "auto" }}>
              <p className="mb-2 text-sm font-semibold text-center text-gray-700">{row[0]}</p>
              <div className="w-full h-[200px]">
                <Bar data={{ labels: years, datasets: [{ data: values, backgroundColor: generateColors(values.length), borderRadius: 5 }] }} options={{ ...baseChartOptions, plugins: { ...baseChartOptions.plugins, legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { font: { size: 11 } } } } }} />
              </div>
            </div>
          );
        })
        ...
        <div className="px-4 py-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full py-2 pr-3 text-xs border border-gray-200 rounded-lg pl-9 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        </div>
        ...
        <div className="flex-1 px-3 pb-4 overflow-y-auto">
          {loadingFiles ? (
            <div className="flex flex-col gap-2 mt-2">
              {[1,2,3].map(i => <div key={i} className="bg-gray-100 h-14 rounded-xl animate-pulse" />)}
            </div>
            ...
            <ul className="space-y-1.5 mt-1">
              {filteredFiles.map((f) => (
                <li
                  ...
                  className={`group flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-all ${selectedFile?.id === f.id ? "bg-blue-50 border border-blue-200 shadow-sm" : "hover:bg-gray-50 border border-transparent"}`}
                >
      ...
      <main className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="flex items-center justify-between px-6 py-3.5 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="max-w-md text-sm font-bold text-gray-800 truncate">
                {selectedFile?.originalName || "Select a report"}
              </h1>
                      ...
                      idx === 0 ? (
                        (() => {
                          ...
                          return (
                            <div className="mb-8">
                              <div
                                className="pb-4 mb-4 text-2xl font-bold text-gray-900 border-b-2 border-blue-600"
                                dangerouslySetInnerHTML={{ __html: firstLine }}
                              />
                              {rest.trim() && (
                                <div className="leading-relaxed prose-sm prose text-gray-700 max-w-none" dangerouslySetInnerHTML={{ __html: rest }} />
                              )}
                            </div>
                          );
                        })()
                        ...
                        <div
                          className="prose prose-sm max-w-none text-gray-700 leading-relaxed
                            ...
                            prose-strong:text-gray-800 prose-li:my-0.5 prose-p:my-2"
          ...
          <div className="w-full max-w-md overflow-hidden bg-white shadow-2xl rounded-2xl">
            ...
            <form onSubmit={submit} className="flex flex-col gap-4 p-6">
              ...
              <div
                ...
                className={`relative flex flex-col items-center justify-center gap-3 p-8 border-2 border-dashed rounded-xl cursor-pointer transition-all ${dragOver ? "border-blue-500 bg-blue-50" : file ? "border-green-400 bg-green-50" : "border-gray-200 hover:border-blue-400 hover:bg-blue-50/50"}`}
              >
                ...
                {file ? (
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-green-500 mt-0.5">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                    </div>
                    <button type="button" onClick={(e) => { e.stopPropagation(); setFile(null); }} className="text-xs text-red-400 underline hover:text-red-600">Remove</button>
                  </>
                ) : (
                  <>
                    ...
                    <div className="text-center">
                      ...
                      <p className="mt-1 text-xs text-gray-400">Annual Reports, DRHP, Concall transcripts</p>
                    </div>
                  </>
                )}
              </div>
              ...
              {uploadProgress && (
                <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 rounded-xl">
                  <div className="flex-shrink-0 w-4 h-4 border-2 border-blue-600 rounded-full border-t-transparent animate-spin" />
              ...
              <div className="flex items-start gap-3 px-4 py-3 border bg-amber-50 rounded-xl border-amber-100">
                ...
                <p className="text-xs text-amber-700">Processing may take 1–3 minutes depending on report size. The AI will extract all financial data, charts, and key metrics.</p>
              </div>

              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={() => { setShowUpload(false); setFile(null); setUploadProgress(""); }}
                ...
                <button
                  ...
                  className="flex-1 py-2.5 text-sm font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                >
